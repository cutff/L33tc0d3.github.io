...
Strings are being assembled by adding them together.  I recognized that the ```"_IDE" + "_Recorder"``` string probably refers to the ```Selenium_IDE_Recorder```, which would imply that the variable ```_0x9e50xf``` is equal to "elenium".  Instead of doing guesswork though, I decided that I would just find the original definitions of these variables and replace their values.  I found them below the function for creating a XMLHttpRequest object.
